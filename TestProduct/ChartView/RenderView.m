...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self){
        ...
        self.labelFont = [UIFont boldSystemFontOfSize:MAX((int)self.pieRadius/15, 5)];
        _labelRadius = _pieRadius/4*3;
...
- (id)initWithCoder:(NSCoder *)aDecoder{
    ...
    if(self){
        ...
        self.labelFont = [UIFont boldSystemFontOfSize:MAX((int)self.pieRadius/15, 5)];
        _labelRadius = _pieRadius/4*3;
        _selectedSliceOffsetRadius = MAX(10, _pieRadius/15);
...
- (void)setShowPercentage:(BOOL)showPercentage{
    ...
    for(SliceLayer *layer in _pieView.layer.sublayers){
        ...
        if(_showPercentage)
            ...
            label = [NSString stringWithFormat:@"%0.1f%%", layer.value];
        CGSize size = [label sizeWithAttributes:@{NSFontAttributeName:self.labelFont}];
        
        if(M_PI*2*_labelRadius*layer.percentage * 1.5 < MAX(size.width,size.height)){
            [textLayer setString:@""];
        }else{
...
- (void)reloadData{
    if (_dataSource && !_animationTimer){
        ...
        [CATransaction setCompletionBlock:^{
            ...
            //圆盘转动完成后  发送通知, 让tableView(scrollView)恢复转动
            [[NSNotificationCenter defaultCenter]postNotificationName:@"changeUserEnabled" object:nil];
        }];
...
- (SliceLayer *)createSliceLayer
{
    ...
    CFRelease(font);
    [textLayer setFontSize:self.labelRadius/5];
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value
{
    ...
    NSString *label;
    if(_showPercentage)
        label = [NSString stringWithFormat:@"%0.1f%%", pieLayer.percentage*100];
    else
        label = [NSString stringWithFormat:@"%0.1f%%", value];
    CGSize size = [label sizeWithAttributes:@{NSFontAttributeName:self.labelFont}];
    
    [CATransaction setDisableActions:YES];
    if(M_PI*2*_labelRadius*pieLayer.percentage * 1.6 < MAX(size.width,size.height) || value <= 0){
        [textLayer setString:@""];
    }else{
        [textLayer setString:label];
        [textLayer setBounds:CGRectMake(0, 0, size.width, size.height)];
